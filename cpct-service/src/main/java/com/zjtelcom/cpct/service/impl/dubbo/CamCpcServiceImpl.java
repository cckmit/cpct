...
import static com.zjtelcom.cpct.enums.FilterRuleType.EXPRESS;
import static com.zjtelcom.cpct.enums.FilterRuleType.PERTURBED;
import static com.zjtelcom.cpct.enums.FilterRuleType.PRODUCT_MUTEX;
import static com.zjtelcom.cpct.enums.Operator.BETWEEN;
...
>>>>>>> origin/pst
    ...
    ConcurrentHashMap<String, String> privateParams = new ConcurrentHashMap();

    ConcurrentHashMap<String, Object> nonPassedMsg = new ConcurrentHashMap<>();

    boolean nonPassedMsgFlag = false;
    /**
     * 活动级别验证
     */
    @Override
    public Map<String, Object> ActivityCpcTask(Map<String, String> params, Long activityId, Map<String, String> privateParams, Map<String, String> laubelItems, List<Map<String, Object>> evtTriggers, List<Map<String, Object>> strategyMapList, DefaultContext<String, Object> context) {
        ...
        this.privateParams.putAll(privateParams);
...
=======
        List<Map<String, Object>> itgTriggers = filterRuleCheck(activityId, esJson, params);
        ...
        List<Future<Map<String, Object>>> threadList = eachStrategyMapList(strategyMapList, hashMap);
        //获取结果
        try {
            ruleList = putFuture2RuleList(threadList, ruleList, params, esJson, activityId);
            activity.put("nonPassedMsg", nonPassedMsg);
            if (isWithDefaultLabel()) {
                // 判断是否有命中
                if (ruleList.size() > 1) {
                    ruleList = removeDefaultLabel4Result(ruleList, strategyMapList);
                }
                itgTriggers = getISaleDisplay(mktCampaign, itgTriggers);
                ruleList = putISaleDisplay2Result(params, ruleList, itgTriggers);
...
//            executorService.shutdownNow();
    ...
    public List<Map<String, Object>> putFuture2RuleList(List<Future<Map<String, Object>>> threadList, List<Map<String, Object>> ruleList, Map<String, String> params, JSONObject esJson, Long activityId) {
    ...
    public List<Map<String, Object>> removeDefaultLabel4Result(List<Map<String, Object>> ruleList, List<Map<String, Object>> strategyMapList) {
        List<String> ruleIdList = new ArrayList<>();
        for (Map<String, Object> rule : ruleList) {
            Long ruleId = Long.valueOf(rule.get("nowRuleId") == null ? "0" : rule.get("nowRuleId").toString());
            ruleIdList.add(ruleId.toString());
        }
...
=======
    ...
    //将iSale展示列的值放入返回结果
    public List<Map<String, Object>> putISaleDisplay2Result(Map<String, String> params, List<Map<String, Object>> ruleList, List<Map<String, Object>> itgTriggers) {
    ...
    public List<Map<String, Object>> getISaleDisplay(MktCampaignDO mktCampaign, List<Map<String, Object>> itgTriggers) {
    ...
    public boolean isWithDefaultLabel() {
        ...
        return isWithDefaultLabel;
    }


    public List<Future<Map<String, Object>>> eachStrategyMapList(List<Map<String, Object>> strategyMapList, HashMap<String, Object> hashMap) {
        List<Future<Map<String, Object>>> threadList = new ArrayList<>();
>>>>>>> origin/pst
    ...
    // 过滤规则校验
    public List<Map<String, Object>> filterRuleCheck(Long activityId,  JSONObject esJson, Map<String, String> params) {
        List<Map<String, Object>> itgTriggers = new ArrayList<>();
        Map<String, Object> filterRuleIdsRedis = eventRedisService.getRedis("MKT_FILTER_RULE_IDS_", activityId);
        List<Long> filterRuleIds = new ArrayList<>();
        if (filterRuleIdsRedis != null) {
        ...
        if (filterRuleIds != null && filterRuleIds.size() > 0) {
            ...
            for (Long filterRuleId : filterRuleIds) {
                ...
                //判断过滤类型(红名单，黑名单)
                if (filterRule != null) {
                    if (PRODUCT_MUTEX.equals(filterRule.getFilterType())) {  //销售品过滤
                        productMutex4FilterRule(filterRule, esJson, params, activityId);
                        if (nonPassedMsgFlag)
                            return null;
                    } else if (EXPRESS.equals(filterRule.getFilterType())) {  //表达式过滤
                        //暂不处理
                    } else if (PERTURBED.equals(filterRule.getFilterType())) {  //过扰规则
                        itgTriggers.add(perturbed4FilterRule(filterRule));
                    }
                }
            }
        }
    ...
    public void productMutex4FilterRule(FilterRule filterRule, JSONObject esJson, Map<String, String> params, Long activityId) {
        boolean productCheck = true; // 默认拦截
        //获取需要过滤的销售品
        String checkProduct = filterRule.getChooseProduct();
        String s = sysParamsService.systemSwitch("PRODUCT_FILTER_SWITCH");
        ...
        if (checkProduct != null && !"".equals(checkProduct)) {
            ...
            // 判断是否进行CRM销售品过滤
            if (realProdFilter != null && "1".equals(realProdFilter)) {
                productStr = crmProductFiltering(filterRuleTimeMap);
            } else if (!context.containsKey("PROM_LIST")) { // 有没有办理销售品--销售列表标签
                //存在于校验
                if ("2000".equals(filterRule.getOperator())) { // 不存在
                    productCheck = false;
    ...
    public String crmProductFiltering(Map<String, Object> filterRuleTimeMap) {
        String productStr = "";
        log.info("111------accNbr --->" + privateParams.get("accNbr"));
        List<String> prodList = new ArrayList<>();
        CacheResultObject<Set<String>> prodInstIdsObject = iCacheProdIndexQryService.qryProdInstIndex2(privateParams.get("accNbr"));
        ...
        if (prodInstIdsObject != null && prodInstIdsObject.getResultObject() != null) {
            Set<String> prodInstIds = prodInstIdsObject.getResultObject();
            String next = prodInstIds.iterator().next();
            CacheResultObject<ProdInst> prodInstCacheEntity = iCacheProdEntityQryService.getProdInstCacheEntity(next);
            if(prodInstCacheEntity!=null && prodInstCacheEntity.getResultObject()!=null&&prodInstCacheEntity.getResultObject().getMainOfferInstId()!=null){
                // 查询销售品实例缓存实体
                CacheResultObject<OfferInst> offerInstCacheEntity = iCacheOfferEntityQryService.getOfferInstCacheEntity(prodInstCacheEntity.getResultObject().getMainOfferInstId().toString());
                log.info("333------offerInstCacheEntity --->" + JSON.toJSONString(offerInstCacheEntity));
                if (offerInstCacheEntity != null && offerInstCacheEntity.getResultObject() != null) {
                    OfferInst offerInst = offerInstCacheEntity.getResultObject();
                    log.info("444------offer --->" + JSON.toJSONString(offerInst));
                    prodList.add(offerInst.getOfferId().toString());
                    filterRuleTimeMap.put(offerInst.getOfferId().toString(), offerInst.getEffDate());
                    log.info("555------filterRuleTimeMap --->" + JSON.toJSONString(filterRuleTimeMap));
                }
            }
...
//            }
    ...
    public Map<String, Object> perturbed4FilterRule(FilterRule filterRule) {
        //将过扰规则的标签放到iSale展示列
        //获取过扰标签
        Map<String, Object> labelsRedis = eventRedisService.getRedis("FILTER_RULE_DISTURB_" + filterRule.getRuleId());
        List<String> labels = new ArrayList<>();
        if (labelsRedis != null) {
            labels = (List<String>) labelsRedis.get("FILTER_RULE_DISTURB_" + filterRule.getRuleId());
            if (labels == null) {
                //过滤规则信息查询失败
                /*esJson.put("hit", false);
                esJson.put("msg", "过扰规则信息查询失败 byId: " + filterRuleId);
...
>>>>>>> origin/pst
        ...
        List<Map<String, Object>> triggerList = containsPerturbedLabelCheck(labels);
...
=======
    public List<Map<String, Object>> containsPerturbedLabelCheck(List<String> labels) {
        List<Map<String, Object>> triggerList = new ArrayList<>();
        if (labels != null && labels.size() > 0) {
            for (String labelCode : labels) {
                if (context.containsKey(labelCode)) {

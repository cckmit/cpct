...
//                String[] codeList = content.split("\\|@\\|");
                ...
                if (i==100000 || j ==0){
                    ...
                    map.put("stringList",list1);
                    new Thread(){
                        @Override
                        public void run() {
                            TrialResponseES responseES = restTemplate.postForObject("http://localhost:30313/es/search", map, TrialResponseES.class);
                            List<Map<String,Object>> list = (List<Map<String,Object>>) responseES.getHitsList().get("result");
...
//创建工作表sheet
        ...
        for (int i = 0; i < list.size(); i++) {
            if ((i + 1) % 65535 == 0) {
                ...
                row.createCell(2).setCellValue("信用2");
                index++;
            }
            row = sheet.createRow((i + 1) - (index * 65535));
// 第四步，创建单元格，并设置值
            row.createCell((short) 0).setCellValue(list.get(i).get("ASSET_INTEG_ID").toString());
            row.createCell((short) 1).setCellValue(list.get(i).get("PROM_AGREE_EXP_TYPE").toString());
            row.createCell((short) 2).setCellValue(list.get(i).get("LATN_NAME").toString());
        }
//写入数据
//        for (int i=0;i<list.size();i++){

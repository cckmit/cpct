...
public class TryCalcServiceImpl implements TryCalcService {
    ...
    public ValidateResult validate(String serialNum, CalcReqModel calcReqModel) {
        ...
        IDacher<Label> labelCache = CacheManager.getInstance().getCache(
                CacheConstants.TRIGGER_CACHE_NAME);
        IDacher<LabelValue> labelValueCache = CacheManager.getInstance().getCache(
        ...
        for (Map<String, Object> policy : policyList) {
            if (policy.get("recommendType") != null) {
                recommendType = policy.get("recommendType").toString();
            }
            if (policy.get("recommendName") != null) {
                recommendName = policy.get("recommendName").toString();
            }
            ...
            if (policy.containsKey("place")
                ...
                for (String place : places) {
                    ...
                    LabelValue realValue = labelValueCache.queryOne(new StringBuilder("100").append(
            ...
            if (ruleList != null) {
                ...
                for (Map<String, Object> rule : ruleList) {
                    ...
                    flag = validateTag(tagCache, labelCache, resultMessage, recommendType, triggers);

                    //校验通过继续校验协同规则
                    if (flag) {
                        resultMessage = new StringBuilder();
                        resultMessage.append(rule.get("ruleName")).append("协同规则选择的：");
                        flag = validateTag(tagCache, labelCache, resultMessage, recommendType,
                                xietong);
                    }
            ...
            if(ruleList != null){
                for (Map<String, Object> rule : ruleList) {
                    ...
                    for (Map tigger : triggers) {
                        ...
                        if (tigger.get("conditionId") != null) {
                            conditionId = tigger.get("conditionId").toString();
                        }
                        String eagleName = labelCache.queryOne(conditionId).getEagleName();
                        ...
                        if (null != tag
                            ...
                            if (tigger.get("conditionName") != null) {
                                eagleMapping.put(eagleName, tigger.get("conditionName").toString());
                            } else {
                                eagleMapping.put(eagleName, null);
                            }
                        }
                    }
                    ...
                    if (CollectionUtils.isNotEmpty(tagList)) {
                        ...
                        validateResult.setCode(ErrorCode.NOT_CONFIG_MASTER_SUB.getErrorCode());
        ...
        for (Map<String, Object> policy : policyList) {
            ...
            if(policy.get("policyId") != null){
                policyId = policy.get("policyId").toString();
            }
            ...
            String nowDate = DateUtil.getNowDate("yyyyMMddHHmmssSSS");
    ...
    private boolean validateTag(IDacher<EagleTag> tagCache, IDacher<Label> labelValueCache,
        ...
        if (null != triggers) {
            for (Map tigger : triggers) {
                ...
                String eagleName = labelValueCache.queryOne(conditionId).getEagleName();

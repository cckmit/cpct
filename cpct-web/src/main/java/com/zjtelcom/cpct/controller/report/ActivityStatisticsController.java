...
//                Sheet sheetName = wb.createSheet("明知山有虎，知难而退？");//新建表 数据随销报表
                ...
                String[] title = {"活动名称", "活动状态", "活动类型", "活动编码", "活动主题", " 活动目录", "活动渠道", "活动生效时间", "活动失效时间",
                        "关单规则名称","所属地市","批次编码","派单方式","派单数","外呼数", "处理数", "过扰关单数" ,"成功数","外呼率","转化率",
...
//                                Map map3 = JSONObject.parseObject(jsonObject2.toJSONString(), Map.class);
                            ...
                            if (trialOperation!=null){
                                // 短信过扰差值
                                content[i][37] = trialOperation.getSubNum() ==null ? "0" : trialOperation.getSubNum();
                                // 黑名单过滤个数
                                content[i][38] = trialOperation.getBeforeNum() ==null ? "0" : trialOperation.getBeforeNum();
                                // 销售品过滤个数
                                content[i][39] = trialOperation.getEndNum() ==null ? "0" : trialOperation.getEndNum();
                            } else {
                                // 短信过扰差值
                                content[i][37] = "0";
                                // 黑名单过滤个数
